...
namespace TestHotFix {
    ...
    public class ABManager {
        ...
        public void DownloadVersionFile(Action callback) {
            Debug.Log(GameLauncher.Instance);
        ...
        private IEnumerator DownloadVersionFileCorountine(string versionPath, Action callback) {
            ...
            if (www.isDone) {
                ...
                var serverVersionData = JsonMapper.ToObject<VersionData>(www.text);
                ...
                if (!File.Exists(Application.persistentDataPath + "/" + GetRuntimePlatform() + "/" + VERSIONFILENAME)) {
                    Debug.Log("没有版本文件");
                    // 写入版本文件
                    File.WriteAllBytes(Application.persistentDataPath + "/" + GetRuntimePlatform() + "/" + VERSIONFILENAME, www.bytes);
                    // 加入下载字典
                    ...
                    var localVersionData = JsonMapper.ToObject<VersionData>(File.ReadAllText(Application.persistentDataPath + "/" + GetRuntimePlatform() + "/" + VERSIONFILENAME));
        ...
        private IEnumerator DownloadLoadingABCoroutine(Action downloadCallback) {
            WWW www = new WWW(SERVERABPATH + "/" + GetRuntimePlatform() + "/" + LOADINGABNAME);

            yield return www;

            if (www.isDone) {
                File.WriteAllBytes(Application.persistentDataPath + "/" + GetRuntimePlatform() + "/" + LOADINGABNAME, www.bytes);
                ...
                var xx = new DirectoryInfo(Application.persistentDataPath + "/" + GetRuntimePlatform());

                foreach (var item in xx.GetFiles()) {
        ...
        private IEnumerator DownloadAB(Action<float> downloadCallback, Action<float> loadCallback, Action callback) {
            WWW www = new WWW(SERVERABPATH + "/" + GetRuntimePlatform() + "/" + needDownloadABDict.Keys.ToList()[downloadedABIndex]);

            yield return www;

            if (www.isDone) {
                File.WriteAllBytes(Application.persistentDataPath + "/" + GetRuntimePlatform() + "/" + needDownloadABDict.Keys.ToList()[downloadedABIndex] , www.bytes);
